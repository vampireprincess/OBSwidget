...
export default function FieldsConfigPanel() {
  const { codeFiles, fieldValueOverrides, setFieldOverride, resetFieldOverrides, addAsset } = useApp();
  ...
  const ci = "w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-xs text-zinc-200 focus:border-emerald-500 focus:outline-none";
  ...
  function renderField(field: SEField) {
    ...
    switch (type) {
      ...
      case 'font':
        ...
        break;
      case 'button':
        control = <button onClick={() => window.dispatchEvent(new CustomEvent('se-field-button', { detail: { key: field.key } }))} className="w-full px-3 py-1.5 text-xs bg-violet-600 hover:bg-violet-500 text-white rounded transition-colors">{field.label}</button>;
        break;
      default:
  ...
  if (fields.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-zinc-500 px-6 text-center">
        <svg className="w-10 h-10 mb-3 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <p className="text-xs mb-1">No fields defined in this widget.</p>
        <p className="text-[10px] text-zinc-600">Add a FIELDS.json to enable widget settings panel.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-zinc-900">
      ...
      <div className="px-3 py-2 border-b border-zinc-700 shrink-0">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-xs font-semibold text-white">Widget Settings</h3>
          <div className="flex items-center gap-1">
            <button onClick={toggleAll} className="px-2 py-0.5 text-[10px] bg-zinc-800 hover:bg-zinc-700 text-zinc-400 rounded transition-colors">
              {allCollapsed ? 'Expand All' : 'Collapse All'}
            </button>
            {hasOverrides && (
              <button onClick={resetFieldOverrides} className="px-2 py-0.5 text-[10px] bg-zinc-800 hover:bg-red-900/50 text-zinc-400 hover:text-red-400 rounded transition-colors">
                Reset
              </button>
            )}
          </div>
        </div>
        <p className="text-[10px] text-zinc-500">{fields.length} fields · changes apply live</p>
      </div>
          ...
          return (
            <div key={group} className="border-b border-zinc-800 last:border-b-0">
              <button onClick={() => toggleGroup(group)} className="w-full flex items-center justify-between px-3 py-2 text-left hover:bg-zinc-800/50 transition-colors">
                <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wide">{group}</span>
